...
class RoarCompetitionSolution:
    ...
    async def initialize(self) -> None:
        ...
        self.PID = PIDController(-0.01, 0.001, -0.01)
        self.x = 0
        self.y = 0
    ...
    def getWaypoints(self):
        ...
        for i in range(len(self.maneuverable_waypoints)):
            waypoints.append(self.maneuverable_waypoints[i].location[:2])
        return waypoints
    async def step(
        self
    ) -> None:
        ...
        steer_control = self.PID.update(self.compute_error(self.current_waypoint_idx))
        throttle_control = 0.5 * (20 - vehicle_velocity_norm)
        brake_control = -throttle_control
        print(throttle_control, steer_control, self.compute_error(self.current_waypoint_idx))
        control = {
            "throttle": np.clip(throttle_control, 0.0, 1),
            "steer": steer_control,
            "brake": np.clip(brake_control, 0.0, 1),
            "hand_brake": 0.0,
            "reverse": 0,
            "target_gear": 0
        }
